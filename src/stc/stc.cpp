    ...
    case WXTI_TYPE_PREEDIT_COMMITTED:
        {
            ...
            wxLogTrace("stc", "OnTextInput/committed: %s", attrstr.GetString());
    ...
    case WXTI_TYPE_QUERY_PREEDIT_RANGE:
        {
            if (m_swx->pdoc->TentativeActive()) {
                ...
                wxLogTrace("stc", "OnTextInput/QueryPreeditRange result={loc:%d, len:%d}", result.m_location, result.m_length);
            } else {
                evt.Skip();
            }
            break;
    ...
    case WXTI_TYPE_QUERY_SELECTED_RANGE:
        {
            ...
            evt.SetStringResult(wxString::FromUTF8(buffer.data(), buffer.length()));
            wxLogTrace("stc", "OnTextInput/QuerySelectedRange result={loc:%d, len:%d}", result.m_location, result.m_length);
            break;
        }
    case WXTI_TYPE_QUERY_FIRSTRECT_FOR_CHARRANGE:
        {
            ...
            wxLogTrace("stc", "OnTextInput/QueryFirstRectForCharRange {loc:%d, len:%d}", range.GetLocation(), range.GetLength());
            break;
        }
    case WXTI_TYPE_QUERY_SUBSTRING_FROM_RANGE:
        {
            ...
            wxLogTrace("stc", "OnTextInput/QuerySubstringFromRange {loc:%d, len:%d}", range.GetLocation(), range.GetLength());
            break;
        }
    default:
        evt.Skip();
